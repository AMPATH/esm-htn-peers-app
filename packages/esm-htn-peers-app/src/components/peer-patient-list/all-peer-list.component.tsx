...
import {
  DataTableSkeleton,
  Tile,
  Accordion, AccordionItem, Button, ButtonSet
} from 'carbon-components-react';
import {  } from 'carbon-components-react';
import { useTranslation } from 'react-i18next';
import { OpenmrsResource, SessionUser, useLayoutType } from '@openmrs/esm-framework';
...
interface AllPatientListProp {
  user?: SessionUser | OpenmrsResource;
}

const AllPeerPatientList: React.FC<AllPatientListProp> = ({user}) => {
      ...
      <div className={styles.peerPatientsContainer}>
          <Accordion>
            {peers.map((peer, index) => (
              <AccordionItem key={index} open={toggledItem[index]} title={
                <h4>Peer: {peer.display}</h4>
              }>
